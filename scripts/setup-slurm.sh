...
find_nvml_plugin() {
    local candidate
    local search_paths=(
        /usr/lib/slurm/gpu_nvml.so
        /usr/lib64/slurm/gpu_nvml.so
        /usr/lib/x86_64-linux-gnu/slurm/gpu_nvml.so
        /usr/lib/x86_64-linux-gnu/slurm-wlm/gpu_nvml.so
        /usr/lib/aarch64-linux-gnu/slurm/gpu_nvml.so
        /usr/lib/aarch64-linux-gnu/slurm-wlm/gpu_nvml.so
    )
    ...
    candidate=$(ldconfig -p 2>/dev/null | awk '/gpu_nvml\.so/ {print $NF; exit}')
...
HAS_NVML_PLUGIN=false
NVML_PLUGIN_PATH=""
GPU_DEVICE_PATHS=()
GPU_DEVICE_COUNT=0
GPU_GRES_MODE="none"
GPU_SCHED_COUNT=0
HAS_FREEZER_CGROUP=false
PROCTRACK_PLUGIN="proctrack/cgroup"
TASK_PLUGIN_VALUE="task/cgroup,task/affinity"
JOBACCT_PLUGIN="jobacct_gather/cgroup"
...
if command -v nvidia-smi &>/dev/null; then
    ...
    shopt -s nullglob
    GPU_DEVICE_PATHS=(/dev/nvidia[0-9]*)
    shopt -u nullglob
    GPU_DEVICE_COUNT=${#GPU_DEVICE_PATHS[@]}
    if [[ "$GPU_DEVICE_COUNT" -gt 0 ]]; then
        step "Detected GPU device files: ${GPU_DEVICE_PATHS[*]}"
    else
        warn "No /dev/nvidiaN device files found; static GRES fallback may be unavailable."
    fi
else
...
if mount | grep -Eq '[[:space:]]/sys/fs/cgroup/freezer[[:space:]].*[[:space:]]cgroup[[:space:]]'; then
    HAS_FREEZER_CGROUP=true
else
    PROCTRACK_PLUGIN="proctrack/linuxproc"
    TASK_PLUGIN_VALUE="task/affinity"
    JOBACCT_PLUGIN="jobacct_gather/linux"
    warn "cgroup freezer controller is not mounted; falling back to linuxproc/task-affinity plugins."
fi
...
if command -v slurmd &>/dev/null; then
    ...
    if [[ -n "$SLURMD_CAPS" ]]; then
        ...
        SLURMD_BOARDS=$(echo "$SLURMD_CAPS" | sed -n 's/.*Boards=\([^ ]*\).*/\1/p')
        SLURMD_SOCKETS_PER_BOARD=$(echo "$SLURMD_CAPS" | sed -n 's/.*SocketsPerBoard=\([^ ]*\).*/\1/p')
        SLURMD_CORES_PER_SOCKET=$(echo "$SLURMD_CAPS" | sed -n 's/.*CoresPerSocket=\([^ ]*\).*/\1/p')
        SLURMD_THREADS_PER_CORE=$(echo "$SLURMD_CAPS" | sed -n 's/.*ThreadsPerCore=\([^ ]*\).*/\1/p')
        if [[ -n "$SLURMD_NODE_NAME" ]]; then
            NODE_NAME="$SLURMD_NODE_NAME"
        fi
        if [[ -n "$SLURMD_BOARDS" && -n "$SLURMD_SOCKETS_PER_BOARD" && -n "$SLURMD_CORES_PER_SOCKET" && -n "$SLURMD_THREADS_PER_CORE" ]]; then
            NODE_TOPOLOGY="Boards=$SLURMD_BOARDS SocketsPerBoard=$SLURMD_SOCKETS_PER_BOARD CoresPerSocket=$SLURMD_CORES_PER_SOCKET ThreadsPerCore=$SLURMD_THREADS_PER_CORE"
        fi
    fi
fi
...
if [[ "$HAS_GPU" == true ]]; then
    if NVML_PLUGIN_PATH=$(find_nvml_plugin); then
        ...
        GPU_GRES_MODE="nvml"
        GPU_SCHED_COUNT="$GPU_COUNT"
        step "Found Slurm NVML plugin: $NVML_PLUGIN_PATH"
    elif [[ "$GPU_DEVICE_COUNT" -gt 0 ]]; then
        HAS_NVML_PLUGIN=false
        GPU_GRES_MODE="device-files"
        GPU_SCHED_COUNT="$GPU_DEVICE_COUNT"
        warn "Slurm NVML plugin not found. Falling back to static GPU device entries in gres.conf."
    else
        HAS_NVML_PLUGIN=false
        GPU_GRES_MODE="none"
        GPU_SCHED_COUNT=0
        warn "GPU detected via nvidia-smi but no Slurm NVML plugin or /dev/nvidiaN device files found."
        warn "GPU scheduling will be disabled for this setup."
    fi
fi

CONSTRAIN_DEVICES_VALUE="yes"
if [[ "$GPU_GRES_MODE" == "device-files" ]]; then
    # Static GRES device mappings can leave jobs stuck in COMPLETING on some hosts
    # when device cgroup enforcement is enabled.
...
step "CPUs=$CPU_COUNT  RealMemory=${REAL_MEMORY}MB  GPUs=$GPU_COUNT  SchedulableGPUs=$GPU_SCHED_COUNT"
...
if [[ "$GPU_SCHED_COUNT" -gt 0 ]]; then
    PARTITION_NAME="gpu"
    GRES_CONFIG="Gres=gpu:$GPU_SCHED_COUNT"
else
    PARTITION_NAME="cpu"
    GRES_CONFIG=""
fi
...
if [[ -f "$SLURM_CONF" && "$FORCE_RECONFIG" != true ]]; then
    ...
    step "[dry-run] NodeName=$NODE_NAME CPUs=$CPU_COUNT ${NODE_TOPOLOGY:+$NODE_TOPOLOGY }RealMemory=$REAL_MEMORY $GRES_CONFIG"
    ...
    if [[ -n "$NODE_TOPOLOGY" ]]; then
        NODE_LINE="NodeName=$NODE_NAME CPUs=$CPU_COUNT $NODE_TOPOLOGY RealMemory=$REAL_MEMORY"
    fi
...
ProctrackType=$PROCTRACK_PLUGIN
TaskPlugin=$TASK_PLUGIN_VALUE

# Job accounting / completion
# NOTE: accounting_storage/filetxt was removed in Slurm 20.11+.
JobAcctGatherType=$JOBACCT_PLUGIN
...
if [[ -f "$GRES_CONF" && "$FORCE_RECONFIG" != true ]]; then
    ...
    if [[ "$GPU_GRES_MODE" == "nvml" ]]; then
        step "[dry-run] Would write $GRES_CONF with AutoDetect=nvml"
    elif [[ "$GPU_GRES_MODE" == "device-files" ]]; then
        step "[dry-run] Would write $GRES_CONF with static GPU device entries: ${GPU_DEVICE_PATHS[*]}"
    else
...
GRES_EOF
    elif [[ "$GPU_GRES_MODE" == "device-files" ]]; then
        {
            cat <<GRES_HEADER
# gres.conf — generated by setup-slurm.sh
# Falling back to static GPU device entries because Slurm NVML plugin was not found.
GRES_HEADER
            for gpu_dev in "${GPU_DEVICE_PATHS[@]}"; do
                echo "Name=gpu File=$gpu_dev"
            done
        } > "$GRES_CONF"
    else
        cat > "$GRES_CONF" <<GRES_EOF
...
ConstrainDevices=$CONSTRAIN_DEVICES_VALUE
CGROUP_EOF
...
if [[ -f "$OVERRIDE_FILE" && "$FORCE_RECONFIG" != true ]]; then
    ...
    step "[dry-run] extra_hosts: $NODE_NAME → host-gateway (so containers can reach slurmctld)"
else
    ...
    while IFS= read -r lib_line; do
        lib_path=$(echo "$lib_line" | sed -n 's/.*=> \(\/[^ ]*\).*/\1/p' || true)
        if [[ -n "${lib_path:-}" ]]; then
            lib_dir=$(dirname "$lib_path")
            local_found=false
            for existing in "${SLURM_LIB_PATHS[@]+"${SLURM_LIB_PATHS[@]}"}"; do
                [[ "$existing" == "$lib_dir" ]] && local_found=true && break
            done
            [[ "$local_found" == false ]] && SLURM_LIB_PATHS+=("$lib_dir")
        fi
    ...
    if command -v sbatch &>/dev/null; then
        while IFS= read -r ldd_line; do
            lib_path=$(echo "$ldd_line" | sed -n 's/.*=> \(\/[^ ]*\).*/\1/p' || true)
        ...
        done < <(ldd "$(command -v sbatch)" 2>/dev/null || true)
    fi
    ...
    done < <(find /usr/lib /usr/lib64 -name "libslurmfull.so*" 2>/dev/null || true)
    ...
    # Build extra_hosts section so containers can resolve the host's hostname
    # (required for sbatch to reach slurmctld, which binds to $NODE_NAME)
    EXTRA_HOSTS_SECTION="    extra_hosts:
      - \"${NODE_NAME}:host-gateway\""
    FQDN=$(hostname -f 2>/dev/null || true)
    if [[ -n "$FQDN" && "$FQDN" != "$NODE_NAME" ]]; then
        EXTRA_HOSTS_SECTION="${EXTRA_HOSTS_SECTION}
      - \"${FQDN}:host-gateway\""
    fi
...
if [[ "$SKIP_TEST" == true ]]; then
    ...
    POLL_INTERVAL_SEC=2
    step "Waiting for job to complete (up to ${TEST_TIMEOUT_SEC}s)..."
    ELAPSED=0
    JOB_STATE=""
    while [[ $ELAPSED -lt $TEST_TIMEOUT_SEC ]]; do
        JOB_STATE=""
    ...
    if [[ "$JOB_STATE" == "COMPLETED" ]]; then
        ...
        warn "Test job $TEST_JOB_ID remained in COMPLETING after ${TEST_TIMEOUT_SEC}s."
        warn "Proceeding because Slurm services are healthy; this can happen on fresh single-node setups."
        warn "Check later: squeue -j $TEST_JOB_ID"
        ...
        error "Test job did not complete within ${TEST_TIMEOUT_SEC}s. State: ${JOB_STATE:-UNKNOWN}"
...
if [[ "$HAS_GPU" == true ]]; then
    if [[ "$GPU_SCHED_COUNT" -gt 0 ]]; then
        echo "    GPUs:       $GPU_COUNT × $GPU_TYPE (schedulable: $GPU_SCHED_COUNT, mode: $GPU_GRES_MODE)"
    else
        echo "    GPUs:       $GPU_COUNT × $GPU_TYPE (scheduling disabled)"
    fi
else
    echo "    GPUs:       none"
fi
