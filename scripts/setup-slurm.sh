...
# =====================================================================
# Phase 8: Configure accounting
# =====================================================================

info "Phase 8: Configuring accounting..."

ACCT_LOG="/var/log/slurm/accounting.log"

if [[ "$DRY_RUN" == true ]]; then
    step "[dry-run] Would create $ACCT_LOG if missing"
else
    if [[ ! -f "$ACCT_LOG" ]]; then
        touch "$ACCT_LOG"
        step "Created $ACCT_LOG"
    else
        step "$ACCT_LOG already exists."
    fi
    chown slurm:slurm "$ACCT_LOG"
fi
...
if [[ "$SKIP_TEST" == true ]]; then
    ...
    info "Phase 11: [dry-run] Would submit test job and verify with sacct."
else
    ...
    while [[ $ELAPSED -lt 60 ]]; do
        JOB_STATE=$(sacct -j "$TEST_JOB_ID" --format=State --noheader --parsable2 2>/dev/null | head -1 | xargs)
    ...
    if [[ "$JOB_STATE" == "COMPLETED" ]]; then
        ...
        error "Check: sacct -j $TEST_JOB_ID"
        exit 1
    fi

    # Verify sacct returns results (the poller depends on this)
    SACCT_OUTPUT=$(sacct -j "$TEST_JOB_ID" --format=JobID,State --noheader --parsable2 2>/dev/null)
    if [[ -z "$SACCT_OUTPUT" ]]; then
        error "sacct returned no results for job $TEST_JOB_ID. Accounting may not be configured."
        exit 1
    fi
