...
- `runners/` provides pluggable local-executor runner scripts (AlphaFold, Boltz, Chai) and registration helpers.
...
- `cp env.example .env` then edit `.env` (set `GPU_SLOTS` for local dev/host GPUs).
...
- Jobs run through the local Docker executor; configure host GPU availability with `GPU_SLOTS`.
